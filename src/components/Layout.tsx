...
export default function Layout() {
  ...
  return (
    <div className={cn("flex h-[100dvh] w-full overflow-hidden relative transition-colors duration-300", isDark ? "bg-black text-white" : "bg-zinc-50 text-zinc-900")}>
      
      {/* Desktop Sidebar */}
      <aside className={cn("hidden md:flex flex-col w-64 border-r transition-colors duration-300 z-30", isDark ? "bg-[#1c1c1e] border-zinc-800" : "bg-white border-zinc-100")}>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  ...
                  isActive
                    ? isDark ? "bg-zinc-800 text-white font-medium" : "bg-zinc-100 text-black font-medium"
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <nav className={cn("md:hidden absolute bottom-0 w-full px-2 py-2 pb-4 flex justify-around items-center z-20 transition-colors duration-300 border-t", isDark ? "bg-[#1c1c1e] border-zinc-800" : "bg-white border-zinc-100 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]")}>
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
